...
///
///     TEST_RUNNER_KETTO_RUN_BENCHMARKS=1 xcodebuild test -configuration Release \
///       -project Ketto.xcodeproj -scheme Ketto -destination 'platform=macOS,arch=arm64' \
///       -only-testing:KettoTests/ExportThroughputTests
///
...
final class ExportThroughputTests: XCTestCase {
    private static let enabled = ProcessInfo.processInfo.environment["KETTO_RUN_BENCHMARKS"] == "1"
    private static let seconds = 120.0
    private static let sourceFPS = 60

    override func setUpWithError() throws {
        try XCTSkipUnless(Self.enabled, "set KETTO_RUN_BENCHMARKS=1 to run the export throughput benchmark")
    }
    ...
    private func measureExport(sourceWidth: Int, sourceHeight: Int, resolution: ExportSettings.Resolution) async throws -> Double {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("KettoBenchmark-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }

        let bundle = try RecordingBundle.create(at: directory.appendingPathComponent("Benchmark.ketto"))
