...
/// Run with `TEST_RUNNER_KETTO_UPDATE_GOLDEN=1 xcodebuild test …` to (re)record the references.
final class GoldenFrameTests: XCTestCase {
    private static let updateGolden = ProcessInfo.processInfo.environment["KETTO_UPDATE_GOLDEN"] == "1"
    ...
    private func compareWithGolden(_ image: BGRAImage, name: String, file: StaticString = #filePath, line: UInt = #line) throws {
        ...
        guard let data = try? Data(contentsOf: url), let reference = BGRAImage(pngData: data) else {
            XCTFail("Missing golden image \(url.lastPathComponent); record it with KETTO_UPDATE_GOLDEN=1", file: file, line: line)
            return
        }
