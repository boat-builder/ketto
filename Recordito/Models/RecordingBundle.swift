...
extension UTType {
    static let recorditoProject = UTType(exportedAs: "com.recordito.project", conformingTo: .package)
}
...
enum RecordingBundleError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notADirectory(let url): return "\(url.lastPathComponent) is not a Recordito project."
        case .missingScreenRecording(let url): return "\(url.lastPathComponent) has no screen recording."
        }
    }
}

/// A `Name.recordito/` package directory. Source media is never rewritten; all edits live in `edit.json`.
struct RecordingBundle: Equatable, Hashable, Sendable {
    static let pathExtension = "recordito"
...
enum ProjectLibrary {
    static var defaultDirectory: URL {
        ...
        return movies.appendingPathComponent("Recordito", isDirectory: true)
    }
