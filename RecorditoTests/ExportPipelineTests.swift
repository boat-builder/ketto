...
final class ExportPipelineTests: XCTestCase {
    ...
    private func makeBundle(in directory: URL) async throws -> (RecordingBundle, EventsDocument, EditDocument) {
        let bundle = try RecordingBundle.create(at: directory.appendingPathComponent("Synthetic.recordito"))
    ...
    func testExportsSyntheticRecordingToMP4() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("RecorditoExportTests-\(UUID().uuidString)")
    ...
    func testPresetsKeepCanvasAspect() {
        ...
        XCTAssertLessThanOrEqual(landscape.videoBitrate, 60_000_000)
